...
export default function LocalizedCategories({ locale }: Props) {
  ...
  return (
    <section
      id="categories"
      className="relative overflow-hidden border-y border-slate-200 bg-slate-50 px-6 py-14 lg:py-18"
    >
      ...
      <div className="relative mx-auto max-w-7xl">
        ...
        <div className="mt-11 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
          {categories.map((category) => {
            ...
            const card = (
              <article
                className={`group relative h-full overflow-hidden rounded-[24px] border border-slate-200 bg-white p-6 shadow-sm transition duration-300 ${
                  category.comingSoon
                    ? "opacity-80"
              ...
              >
                ...
                <div className="flex items-start justify-between gap-4">
                  <div
                    className={`flex h-12 w-12 items-center justify-center rounded-2xl text-3xl ring-1 ${styles.icon}`}
                  >
                    {category.icon}
                  </div>
