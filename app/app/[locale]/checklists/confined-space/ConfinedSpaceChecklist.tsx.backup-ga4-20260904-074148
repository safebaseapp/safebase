...
export default function ConfinedSpaceChecklist({ locale }: Props) {
  ...
  function runSafetyAnalysis() {
    ...
    try {
      ...
      const analysisResult = analyzeConfinedSpaceChecklist(
        formattedAnswers,
        locale,
      );
      ...
      setAnalysisError(
        error instanceof Error
          ? error.message
          : locale === "tr"
            ? "Güvenlik analizi oluşturulamadı."
            : "Safety analysis could not be generated.",
      );
    }

    window.setTimeout(() => {
      document
        .querySelector("[data-checklist-analysis]")
        ?.scrollIntoView({
          behavior: "smooth",
          block: "start",
        });
        ...
        <section className="hidden print:block sernem-print-header">
          ...
          <div className="mb-5 px-1">
            <h1 className="text-[22px] font-extrabold leading-tight text-[#071a38]">
              {locale === "tr"
                ? "KAPALI ALAN GİRİŞ DENETİM RAPORU"
                : "CONFINED SPACE ENTRY INSPECTION REPORT"}
            </h1>
...
<section className="mt-8 grid gap-4 rounded-3xl border border-slate-800 bg-slate-900 p-6 md:grid-cols-2 xl:grid-cols-3 print:border-slate-300 print:bg-white">
        ...
        <div data-checklist-analysis className="scroll-mt-8 print:hidden">
          {analysisOpened && (
            <div className="mt-8">
              {analysis ? (
                <ChecklistAnalysisPanel
                  locale={locale}
                  analysis={analysis}
                />
              ) : (
                <section className="rounded-3xl border border-amber-500/30 bg-amber-500/10 p-7 text-amber-100">
                  <p className="text-sm font-semibold uppercase tracking-[0.18em]">
                    {locale === "tr"
                      ? "Güvenlik Analizi"
                      : "Safety Analysis"}
                  ...
                  <p className="mt-3 leading-7">
                    {analysisError ||
                      (locale === "tr"
                        ? `${answeredCount}/${items.length} madde cevaplandı. ${noCount} uygunsuzluk ve ${criticalFailures.length} kritik başarısızlık tespit edildi.`
                        : `${answeredCount}/${items.length} items answered. ${noCount} non-compliances and ${criticalFailures.length} critical failures identified.`)}
                  </p>
                </section>
              )}
            </div>
