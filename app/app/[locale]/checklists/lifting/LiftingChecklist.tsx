...
export default function LiftingChecklist({ locale }: Props) {
  ...
  function runSafetyAnalysis() {
    trackEvent("ai_assessment_used", {
      assessment_type: "lifting_checklist",
      locale,
    ...
    try {
      ...
      const result = analyzeLiftingChecklist(formattedAnswers, locale);

      setAnalysis(result);
      setProfessionalAssessment(null);
      setIsAiLoading(false);
    } catch (error) {
      console.error("Lifting analysis error:", error);
      setAnalysis(null);
      ...
      setAnalysisError(
        error instanceof Error
          ? error.message
          : locale === "tr"
            ? "Kaldırma operasyonu güvenlik analizi oluşturulamadı."
            : "Lifting operations safety analysis could not be generated.",
      );
    }
  ...
  async function generateAiAssessment() {
    trackEvent("premium_ai_assessment_used", {
      assessment_type: "lifting",
      locale,
      answered_count: answeredCount,
      total_items: items.length,
      findings_count: noCount,
      critical_findings: criticalFailures.length,
      source: "lifting_premium_ai",
    });
    ...
    try {
      const result = await generateAssessment({
        workType: analysis.checklistTitle || "Lifting Operations",
        language: locale,
      ...
      console.error("Lifting Operations AI Assessment Error:", error);
  ...
  function printInspection() {
    trackEvent("pdf_downloaded", {
      document_type: "lifting_inspection",
      locale,
        ...
        <section className="hidden print:block sernem-print-header">
          ...
          <div className="mb-5 px-1">
            <h1 className="text-[22px] font-extrabold leading-tight text-[#071a38]">
              {locale === "tr"
                ? "KALDIRMA OPERASYONU DENETİM RAPORU"
                : "LIFTING OPERATION INSPECTION REPORT"}
            </h1>
              ...
              {analysis ? (
                ...
                <section className="rounded-3xl border border-amber-500/30 bg-slate-900 p-7 text-white">
                  <p className="text-sm font-semibold uppercase tracking-[0.18em] text-amber-300">
                    {locale === "tr"
                      ? "Kaldırma Operasyonu Güvenlik Analizi"
                      : "Lifting Operations Safety Analysis"}
                  </p>
