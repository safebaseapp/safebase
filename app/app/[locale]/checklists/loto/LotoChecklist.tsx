...
export default function LotoChecklist({ locale }: Props) {
  ...
  function runSafetyAnalysis() {
    trackEvent("ai_assessment_used", {
      assessment_type: "loto_checklist",
      locale,
    ...
    try {
      ...
      const result = analyzeLotoChecklist(formattedAnswers, locale);

      setAnalysis(result);
      setProfessionalAssessment(null);
      setIsAiLoading(false);
    } catch (error) {
      console.error("LOTO analysis error:", error);
      setAnalysis(null);
      ...
      setAnalysisError(
        error instanceof Error
          ? error.message
          : locale === "tr"
            ? "LOTO güvenlik analizi oluşturulamadı."
            : "LOTO safety analysis could not be generated.",
      );
    }
  ...
  async function generateAiAssessment() {
    trackEvent("premium_ai_assessment_used", {
      assessment_type: "loto",
      locale,
      answered_count: answeredCount,
      total_items: items.length,
      findings_count: noCount,
      critical_findings: criticalFailures.length,
      source: "loto_premium_ai",
    });
    ...
    try {
      const result = await generateAssessment({
        workType: analysis.checklistTitle || "LOTO",
        language: locale,
      ...
      console.error("LOTO AI Assessment Error:", error);
        ...
        <section className="hidden print:block sernem-print-header">
          ...
          <div className="mb-5 px-1">
            <h1 className="text-[22px] font-extrabold leading-tight text-[#071a38]">
              {locale === "tr"
                ? "LOTO DOĞRULAMA DENETİM RAPORU"
                : "LOTO VERIFICATION INSPECTION REPORT"}
            </h1>
              ...
              {analysis ? (
                ...
                <section className="rounded-3xl border border-amber-500/30 bg-slate-900 p-7 text-white">
                  <p className="text-sm font-semibold uppercase tracking-[0.18em] text-amber-300">
                    {locale === "tr"
                      ? "LOTO Güvenlik Analizi"
                      : "LOTO Safety Analysis"}
                  </p>
